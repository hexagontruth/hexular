var Hexular = (function () {

  // --- DEFAULTS ---

  const DEFAULT_ROWS = 60;
  const DEFAULT_COLS = 60;
  const DEFAULT_NUM_STATES = 2;
  const DEFAULT_MAX_STATES = 12;
  const DEFAULT_RULE = nullRule;

  const DEFAULT_RADIUS = 10;
  const DEFAULT_BORDER_WIDTH = 1;
  const DEFAULT_HIGHLIGHT_COLOR = '#ffbb33';
  const DEFAULT_HIGHLIGHT_LINE_WIDTH = 2;

  const DEFAULT_TIMER_LENGTH = 100;

  var DEFAULT_COLORS = [
    '#ffffff',

    '#cccccc',
    '#999999',
    '#666666',
    '#333333',

    '#cc4444',
    '#ee7722',
    '#eebb33',
    '#66bb33',
    '#66aaaa',
    '#4455bb',
    '#aa55bb'
  ];

  const CANVAS_CLASS = 'hexular-canvas';

  var defaults = {
    rows: DEFAULT_ROWS,
    cols: DEFAULT_COLS,
    numStates: DEFAULT_NUM_STATES,
    maxStates: DEFAULT_MAX_STATES,
    defaultRule: DEFAULT_RULE,
    colors: DEFAULT_COLORS,
    radius: DEFAULT_RADIUS,
    borderWidth: DEFAULT_BORDER_WIDTH,
    highlightColor: DEFAULT_HIGHLIGHT_COLOR,
    highlightLineWidth: DEFAULT_HIGHLIGHT_LINE_WIDTH,
    timerLength: DEFAULT_TIMER_LENGTH
  };

  var index = 0;

  // --- MATH STUFF ---

  const APOTHEM = Math.sqrt(3) / 2;

  var math = {
    tau: Math.PI * 2,
    arc: Math.PI / 3,
    apothem: APOTHEM,
    inverseApothem: 1 / APOTHEM,
    vertices: [
      [-1, 0],
      [-0.5, -APOTHEM],
      [0.5, -APOTHEM],
      [1, 0],
      [0.5, APOTHEM],
      [-0.5, APOTHEM]
    ],
    basis: [
      1.5, 0,
      APOTHEM, 2 * APOTHEM
    ],
    invBasis: [
      2 / 3, 0,
      -1 / 3, 1 / (2 * APOTHEM)
    ]
  };

  // --- MAIN CONSTRUCTOR ---

  Hexular.prototype = defaults;

  function Hexular() {
    // Parse arguments

    let opts = {};
    this.rules = [];

    for (let i = 0; i < arguments.length; i++) {
      if (typeof arguments[i] == 'function')
        this.rules.push(arguments[i]);
      else if (typeof arguments[i] == 'object')
        opts = arguments[i];
    }

    // Assign opts

    for (let k in opts)
      if (opts.hasOwnProperty(k))
        this[k] = opts[k];

    this.colors = this.colors.slice();

    // Populate default rules

    for (let i = this.rules.length; i < this.maxStates; i++)
      this.rules.push(this.defaultRule);

    // Other state properties

    this.index = index++;

    this.timer = null;
    this.running = false;

    this.selected = {
      cell: null,
      imageData: null,
      y: 0,
      x: 0
    };

    // Useful local aliases

    const radius = this.radius;
    const rows = this.rows;
    const cols = this.cols;

    // Canvas

    this.canvas = document.createElement('canvas');
    this.context = this.canvas.getContext('2d');

    this.canvas.height  = 2 * radius * (rows + 1.5) * 0.75;
    this.canvas.width = 2 * radius * (cols + 1.5) * math.apothem;

    this.canvas.classList.add(CANVAS_CLASS);

    // Precomputed math stuff

    this.innerRadius = radius - this.borderWidth / (2 * math.apothem);
    this.vertices = elemOp(math.vertices, this.innerRadius);

    this.basis = elemOp(math.basis, radius);
    this.invBasis = elemOp(math.invBasis, 1 / radius);

    this.yOffset = 2 * radius;
    this.xOffset = 2 * radius * math.apothem;

    // For imageData rectangle coords
    this.selectYOffset = Math.ceil(
      radius * math.apothem + this.highlightLineWidth);
    this.selectXOffset = Math.ceil(
      radius + this.highlightLineWidth);
    this.selectHeight = this.selectYOffset * 2;
    this.selectWidth = this.selectXOffset * 2;

    // Callback hooks for drawing actions

    this.onDrawCell = new HookList(this);
    this.onDrawCell.push(defaultDrawCell);

    this.onDrawSelector = new HookList(this);
    this.onDrawSelector.push(defaultDrawSelector)

    // Initialize cells

    this.cells = new Array(rows);
    for (let i = 0; i < rows; i++)
      this.cells[i] = new Array(cols);

    this.eachCoord(function(u, v, thisArg) {
      thisArg.cells[u][v] = new Cell(thisArg, u, v);
    });

    // Connect cells
    for (let i = 0; i < rows; i++) {
      let upRow = mod(i - 1, rows);
      let downRow = mod(i + 1, rows);
      let offset = downRow % 2;
      for (let j = 0; j < cols; j++) {
        const cell = this.cells[i][j];
        cell.neighbors[0] = this.cells[upRow][mod(j - offset, cols)];
        cell.neighbors[1] = this.cells[i][mod(j - 1, cols)];
        cell.neighbors[2] = this.cells[downRow][mod(j - offset, cols)];
        cell.neighbors[3] = this.cells[downRow][mod(j - offset + 1, cols)];
        cell.neighbors[4] = this.cells[i][mod(j + 1, cols)];
        cell.neighbors[5] = this.cells[upRow][mod(j - offset + 1, cols)];
      }
    }
  }

  // Iterate over u,v coords for each valid cell

  Hexular.prototype.eachCoord = function(callback) {
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.cols; j++) {
        if (callback(i, j, this) === false)
          return;
        }
      }
  };

  // Iterate over cells

  Hexular.prototype.eachCell = function(callback) {
    this.eachCoord(function(u, v, thisArg) {
      return callback(thisArg.cells[u][v], thisArg);
    });
  };

  // Draw all hexes

  Hexular.prototype.draw = function() {
    this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);

    this.eachCell(function(cell, thisArg) {
        thisArg.drawCell(cell);
    });

    if (this.selected.cell) {
      this.getImageData();
      this.drawSelector(this.selected.cell);
    }
  };

  // Select cell and highlight on canvas

  Hexular.prototype.selectCell = function(cell) {
    if (this.selected.cell != cell) {
      if (this.selected.cell) {
        this.putImageData()
      }
      if (cell) {
        this.selected.y = cell.y - this.selectYOffset;
        this.selected.x = cell.x - this.selectXOffset;
        this.getImageData();
        this.drawSelector(cell);
      }
      this.selected.cell = cell;
    }
  };

  // Get backup image data for selected cell and store to selected.imageData

  Hexular.prototype.getImageData = function() {
    this.selected.imageData = this.context.getImageData(
      this.selected.x,
      this.selected.y,
      this.selectWidth,
      this.selectHeight);
  }

  // Replace image data when overwriting last selector

  Hexular.prototype.putImageData = function() {
    this.context.putImageData(
      this.selected.imageData,
      this.selected.x,
      this.selected.y);
  }

  // Draw actions

  Hexular.prototype.drawCell = function(cell) {
    this.onDrawCell.call(cell);
  };

  Hexular.prototype.drawSelector = function(cell) {
    this.onDrawSelector.call(cell);
  };

  // Basic hex path for both cells and selector

  Hexular.prototype.drawHexPath = function(cell) {
    const y = cell.y
    const x = cell.x;
    const context = this.context;
    const vertices = this.vertices;

    context.beginPath();
    context.moveTo(x + vertices[0][1], y + vertices[0][0]);

    context.lineTo(x + vertices[1][1], y + vertices[1][0]);
    context.lineTo(x + vertices[2][1], y + vertices[2][0]);
    context.lineTo(x + vertices[3][1], y + vertices[3][0]);
    context.lineTo(x + vertices[4][1], y + vertices[4][0]);
    context.lineTo(x + vertices[5][1], y + vertices[5][0]);

    context.closePath();
  };

  // Get cell at y,x coords on canvas

  Hexular.prototype.cellAtPosition = function(y, x) {
    y -= this.yOffset;
    x -= this.xOffset;

    // We first find the position with respect to cubic coordinates
    var u = this.invBasis[0] * y + this.invBasis[1] * x;
    var v = this.invBasis[2] * y + this.invBasis[3] * x;
    var w = -u - v;

    var ru = Math.round(u);
    var rv = Math.round(v);
    var rw = Math.round(w);

    var du = Math.abs(ru - u);
    var dv = Math.abs(rv - v);
    var dw = Math.abs(rw - w);

    if (du > dv && du > dw)
      ru = -rv - rw;
    else if (du > dw)
      rv = -ru - rw;

    // Every two rows are then shifted one space to the left
    rv += Math.floor(ru / 2);

    if (ru < 0 || ru >= this.rows || rv < 0 || rv >= this.cols)
      return null;
    else
      return this.cells[ru][rv];
  };

  // --- Timer/step stuff ---

  Hexular.prototype.startStop = function() {
    if (this.timer) {
      this.stop();
      return false;
    }
    else {
      this.start();
      return true;
    }
  };

  Hexular.prototype.start = function() {
    this.timer = setInterval(this.step.bind(this), this.timerLength);
    this.running = true;
  };

  Hexular.prototype.stop = function() {
    clearTimeout(this.timer);
    this.timer = null;
    this.running = false;
  };

  Hexular.prototype.step = function() {
    this.eachCell(function(cell, thisArg) {
      cell.nextState = thisArg.rules[cell.state](cell);
    });
    this.eachCell(function(cell) {
      cell.state = cell.nextState;
    });
    this.draw();
  };

  Hexular.prototype.clear = function() {
    this.eachCell(function(cell) {
      cell.state = 0;
    });
    this.draw();
  };

  Hexular.prototype.test = function(n) {
    n = n || 64;

    var a = +new Date();

    for (let i = 0; i < n; i++)
      this.step();

    var b = +new Date();

    return b - a;
  }

  // --- CELLS ---

  function Cell(owner, u, v) {
    this.owner = owner;
    this.u = u;
    this.v = v;

    // Useful for custom drawing actions that involve paths between neighbors
    this.isEdge =
      (u == 0 || u == owner.rows - 1 || v == 0 || v == owner.cols - 1);

    // We again calculate cubic coords and shift x left once every 2 rows
    this.y = owner.yOffset + owner.basis[0] * u + owner.basis[1] * v;
    this.x = owner.xOffset + owner.basis[2] * u + owner.basis[3] *
      (v - Math.floor(u / 2));

    this.state = 0;
    this.nextState = null;
    this.neighbors = Array(6);
  }

  // --- Cell rule helpers ---

  Cell.prototype.total = function() {
    let tot = 0
    for (let i = 0; i < 6; i ++)
      tot += this.neighbors[i].state;
    return tot;
  };

  Cell.prototype.countAll = function() {
    let count = 0
    for (let i = 0; i < 6; i ++)
      if (this.neighbors[i].state > 0)
        count++;
    return count;
  }

  Cell.prototype.count = function(state) {
    let count = 0;
    for (let i = 0; i < 6; i ++)
      if (this.neighbors[i].state == state)
        count ++;
    return count;
  }

  Cell.prototype.counts = function() {
    let values = Array(this.owner.maxStates).fill(0);
    for (let i = 0; i < 6; i ++)
      values[this.neighbors[i].state] += 1;
    return values;
  }

  Cell.prototype.stateMap = function() {
    return this.neighbors.map(function(e) {
      return this.state;
    });
  }

  Cell.prototype.max = function(states) {
    states = states || this.stateMap();
    return Math.max.apply(null, this.stateMap());
  }

  Cell.prototype.min = function(states) {
    states = states || this.stateMap();
    return Math.min.apply(null, this.stateMap());
  }

  Cell.prototype.offset = function(i) {
    return mod(this.state + i, this.owner.numStates);
  }

  // --- CALLBACKS ---

  HookList.prototype = new Array();

  function HookList(owner) {
    this.owner = owner;
  }

  HookList.prototype.call = function() {
    for (let i = 0; i < this.length; i++)
      if (this[i].apply(this.owner, arguments) === false)
        return false;

    return true;
  };

  // --- DEFAULT CELL CALLBACKS ---

  function nullRule(cell) {
    return cell.state;
  }

  function defaultDrawCell(cell) {
    // Use cell.owner when writing custom drawing callbacks
    this.drawHexPath(cell);

    this.context.fillStyle = this.colors[cell.state];
    this.context.fill();
  }

  function defaultDrawSelector(cell) {
    this.drawHexPath(cell);

    this.context.strokeStyle = this.highlightColor;
    this.context.lineWidth = this.highlightLineWidth;
    this.context.stroke();
  }

  // --- UTILITY FUNCTIONS ---

  // Recursive element-wise arithmetic

  function elemOp(obj, scalar, op) {
    if (obj.length)
      return obj.map(function(val, i) {
        return elemOp(val, scalar, op);
      });
    else
      return op == '+' ? obj + scalar : obj * scalar;
  }

  // Mod for reals

  function mod(k, n) {
    return ((k % n) + n) % n;
  }

  // ---

  Hexular.nullRule = nullRule;
  Hexular.defaultDrawCell = defaultDrawCell;
  Hexular.defaultDrawSelector = defaultDrawSelector;

  Hexular.elemOp = elemOp;
  Hexular.mod = mod;

  return Hexular;
})();
